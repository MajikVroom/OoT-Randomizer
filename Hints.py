...
def get_important_check_hint(spoiler: Spoiler, world: World, checked: set[str]) -> HintReturn:
    ...
    for location in world.get_filled_locations():
        if (HintArea.at_important_check(location).text(world.settings.clearer_hints) not in top_level_locations
                ...
                and HintArea.at_important_check(location) != HintArea.ROOT):
        ...
        if hint_type == 'big_poes':
            ...
            if hint_type in world.settings.misc_hints:
                text = data['location_text'].format(item=get_hint(get_item_generic_name(item),
                                                                    world.settings.clearer_hints).text, poe_points=poe_points, optional_player_text=optional_player_text)
            else:
            ...
            if hint_type in world.settings.misc_hints:
                if hint_type in world.misc_hint_location_items:
                    ...
                    text = data['location_text'].format(item=get_hint(get_item_generic_name(item),
                                                                        world.settings.clearer_hints).text, optional_player_text=optional_player_text)
