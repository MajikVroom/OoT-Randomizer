...
def get_important_check_hint(spoiler: Spoiler, world: World, checked: dict[HintArea | str, set[CheckedKind]]) -> HintReturn:
    ...
    for location in locations:
        hint_area = HintArea.at_important_check(location)
    ...
    for location in locations:
        if HintArea.at_important_check(location) == hint_area:
            shuffled_locations_in_region = list(filter(lambda loc: HintArea.at_important_check(loc) == hint_area and not loc.locked, locations))
